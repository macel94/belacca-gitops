...
rules. External status-repository SLO evidence is published separately by the
GitHub-hosted monitor; it is not scraped into Prometheus and native metrics do
not substitute for the external user-journey SLI.
...
- Dashboard and Flux authenticated journeys remain external/configuration-
  unknown until an operator-managed identity is provisioned.
